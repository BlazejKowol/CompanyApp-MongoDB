...
router.post('/employees', (req, res) => {
  ...
  req.db.collection('products').insertOne({ firstName: firstName }, {lastName: lastName}, {department: department}, err => {
    if (err) res.status(500).json({ message: err });
...
router.put('/employees/:id', (req, res) => {
  const { firstName, lastName, department } = req.body;
  req.db.collection('products').updateOne({ _id: ObjectId(req.params.id) }, {$set: { firstName: firstName, lastName: lastName, department: department }}, err => {
    if (err) res.status(500).json({ message: err });
    else res.json({ message: 'OK' });
  });
});
